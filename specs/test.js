...
describe('Coding challenge', function() {
  it('should return 400 with appropriate error on invalid json', function(done) {
    request.post(url, 'invalid json', function(error, response, body) {
      expect(response.statusCode).to.equal(400);
      expect(response.body).to.equal('{"error":"Could not decode request: JSON parsing failed"}');
      done();
    });
  });
  it('should return the right response', function(done) {
    request.post({url: url, json: sampleRequest}, function(error, response, body) {
      expect(response.statusCode).to.equal(200);
      expect(JSON.stringify(response.body)).to.equal(JSON.stringify(sampleResponse));
      done();
    });
  });
});
